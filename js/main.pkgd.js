...
function treeChart(d3) {
  ...
  function chart(selection) {
    ...
    function bakeLayout(d) {
      var containerSel = d3.select(this).selectAll('.chart-container').data(['']);
      chartContainer = containerSel.enter().append('div').classed('chart-container', true).merge(containerSel);

      var svgSel = chartContainer.selectAll('svg').data(['']);
      svg = svgSel.enter().append('svg').merge(svgSel).attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom);

      var gSel = svg.selectAll('g.svg-g').data(['']);
      g = gSel.enter().append('g').classed('svg-g', true).merge(gSel).attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    }
...
inputButton.addEventListener('animationend', function () {
  return inputButton.classList.remove('active');
});
inputButton.addEventListener('webkitAnimationEnd', function () {
  return inputButton.classList.remove('active');
});
